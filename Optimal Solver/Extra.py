    ...
    for problem in p_number:
        ...
        if city == 3:
            for i in range(city*10):
                bryce = i+82
                near = nn(i,coords)
                out = tour_distance(near, coords)[:28, :]
                np.savetxt(f"{city_out}/dist30_sub{bryce}_prob{problem}.csv", out, delimiter=',', fmt='%f',header = l)
                np.savetxt(f"{city_out}/solution/sol{city}0_sub{bryce}_prob{problem}.csv", np.transpose(near)[1:29],
                           delimiter=',', fmt='%f', header='order')
        else:
            for i in range(city*10):
                bryce = i+82
                near = nn(i,coords)
                out = tour_distance(near, coords)[:38, :]
                np.savetxt(f"{city_out}/dist40_sub{bryce}_prob{problem}.csv", out, delimiter=',', fmt='%f',
                           header=l2)
                np.savetxt(f"{city_out}/solution/sol{city}0_sub{bryce}_prob{problem}.csv", np.transpose(near)[1:39],
                           delimiter=',', fmt='%f', header='order')
