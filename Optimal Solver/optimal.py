...
def tour_length(order, xy):
    ...
    for i in range(len(order)):
        if i < max(range(len(order))):
            length += math.dist(xy[order[i]-1],xy[order[i]-1])
        else:
            length += math.dist(xy[order[i]-1], xy[order[0]-1])
    return length

# print(tour_length(e1,p1))
# print(tour_length(e2,p2))
# print(tour_length(e3,p3))
# print(tour_length(e4,p4))
# print(tour_length(e5,p5))
# print(tour_length(e6,p6))
# print(tour_length(e7,p7))
# print(tour_length(e8,p8))
# print(tour_length(e9,p9))
...
optimal = [4.580679247989077,4.429730842249746,4.957412904903573,5.000663472004715,4.660907671010068,4.708294339862599,4.625020023726632,5.154592117209478,4.938040417948793]



coords = [p1,p2,p3,p4,p5,p6,p7,p8,p9]
print(coords[3])
total = np.zeros((9,82))
cit = 3

c = data_big[3][:,1,1]
a = tour_length(c, coords[1])
b = optimal[1]
...
for p in p_number:
    for s in s_number:
        # print(data_big[cit][:,p-1,s-1])
        # print(coords[p-1])
        # print(tour_length(data_big[cit][:,p-1,s-1], coords[p-1]))
        total[p-1,s-1] = tour_length(data_big[cit][:,p-1,s-1], coords[p-1]) / optimal[p-1]

avg = []
for a in range(len(s_number)):
    avg.append(np.mean(total[:,a]))

print(avg)
np.savetxt("optimalavg30.csv", avg, delimiter=',', fmt='%f',header = None)
